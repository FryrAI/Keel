...
use crate::types::{FocusFile, FocusResult, FocusSymbol};
...
impl EnforcementEngine {
    ...
    pub fn focus(&self, target: &str, depth: u32) -> Option<FocusResult> {
        ...
        for node in &targets {
            self.record_symbol(&mut symbols, node, 0, "target");
            ...
            for c in upstream {
                if let Some(n) = self.store.get_node(&c.hash) {
                    self.record_symbol(&mut symbols, &n, c.distance, "caller");
                }
            }
            for c in downstream.into_iter().filter(|c| c.distance == 1) {
                if let Some(n) = self.store.get_node(&c.hash) {
                    self.record_symbol(&mut symbols, &n, 1, "callee");
                }
            }
        }
    ...
    fn build_focus_symbol(&self, node: &GraphNode, distance: u32, relation: &str) -> FocusSymbol {
        let callers = self
            .store
...
fn relation_priority(relation: &str, distance: u32) -> u32 {
    match relation {
        "target" => 0,
        "callee" => 1,
        _ => 1 + distance,
