...
# `keel validate-plan --llm -`, then prints P001 (unknown call target), P002
# (signature mismatch), and advisory-only P003 (reuse candidate) findings to
# stderr, where Claude Code shows them to the model.
#
...
#   Blocking (opt-in):   KEEL_PLAN_STRICT=1 claude    # exit 2 on P001/P002 only
#
# Repeat P001/P002 findings are routed through keel's circuit breaker: after 3 strikes on
...
FINDINGS=$(printf '%s\n' "$RESULT" | grep -E '^(P00[123] |  at: |  fix: )' || true)
[ -z "$FINDINGS" ] && exit 0
